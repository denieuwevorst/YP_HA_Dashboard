#!/bin/bash
# Weekplanner — haalt evenementen + tijdschema + resources op voor de komende 7 dagen
...
OUTPUT_FILE="${OUTPUT_DIR}/weekplanning.json"

# Laad locaties als bash array
mapfile -t LOCATIES < <(jq -r '.locaties[]' "$CONFIG")

RESULT="[]"

for i in 0 1 2 3 4 5 6; do
  DAY_DD=$(date -d "+${i} days" +%d-%m-%Y)
  DAY_ISO=$(date -d "+${i} days" +%Y-%m-%d)

  ALL_EVENTS=$(curl -s "${BASE}/events/date:${DAY_DD}?api_key=${API_KEY}" | jq '
  ...
  for LOCATIE in "${LOCATIES[@]}"; do
    ...
    for j in $(seq 0 $((EVENT_COUNT - 1))); do
      EVENT=$(echo "$ZAAL_EVENTS" | jq ".[$j]")
      ...
      EVENT_WITH_META=$(echo "$EVENT" | jq \
        ...
        '. + {zaal: $zaal, date: $date, schedule: $schedule, resources: $res}')
...
echo "$RESULT" | jq 'sort_by(.date, .zaal, .starttime)' > "$OUTPUT_FILE"
